pageextension 13632 "User Security Activ. Nemhandel" extends "User Security Activities"
{
    // this page is a part of page 9024 Security Admin Role Center

    var
        NemhandelStatusPageBckgrnd: Codeunit "Nemhandel Status Page Bckgrnd";
        NemhandelStatusMgt: Codeunit "Nemhandel Status Mgt.";
        BackgroundTaskId: Integer;
        PageBckGrndTaskCompletedTxt: Label 'User Security Activities Page Background Task to check Nemhandel registration status completed. Status: %1; status text: %2', Comment = '%1, %2 - Registered/NotRegisted/Unknown', Locked = true;
    ...
    begin
        ...
        if Results.Get(NemhandelStatusPageBckgrnd.GetStatusKey(), CompanyStatusTextValue) then
            if Evaluate(CompanyStatus, CompanyStatusTextValue) then begin
                OnAfterGetCompanyStatusUserSecActivitBckgrndTask(CompanyStatus);
        ...
        Telemetry.LogMessage(
            '0000LBA', StrSubstNo(PageBckGrndTaskCompletedTxt, CompanyStatus, CompanyStatusTextValue), Verbosity::Normal,
    ...
    local procedure OnAfterGetCompanyStatusUserSecActivitBckgrndTask(var CompanyStatus: Enum "Nemhandel Company Status")
    begin
    end;
}