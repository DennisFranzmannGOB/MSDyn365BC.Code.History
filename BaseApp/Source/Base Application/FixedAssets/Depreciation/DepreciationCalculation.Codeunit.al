...
codeunit 5616 "Depreciation Calculation"
{
    ...
    begin
        ...
        with FALedgEntry do begin
            ...
            if FindLast() then
                if ToMorrow("FA Posting Date", Year365Days, UseAccountingPeriod) > LocalDate then
                    LocalDate := ToMorrow("FA Posting Date", Year365Days, UseAccountingPeriod);
            GetLastEntryDates(FANo, DeprBookCode, EntryDates);
    ...
    begin
        ...
        with FALedgEntry do begin
            ...
            if StartingDate = 0D then begin
                ...
                SetRange("FA Posting Type", "FA Posting Type"::Depreciation);
                if Find('+') then
                    StartingDate := ToMorrow("FA Posting Date", Year365Days, UseAccountingPeriod)
                else begin
