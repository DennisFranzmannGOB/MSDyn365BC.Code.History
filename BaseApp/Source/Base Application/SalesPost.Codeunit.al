codeunit 80 "Sales-Post"
{
    ...
    begin
        IsHandled := false;
        OnBeforePostSalesDoc(Rec, SuppressCommit, PreviewMode, HideProgressWindow, IsHandled);
        if IsHandled then
            exit;
        ...
        SalesHeader := Rec;
        OnCodeOnBeforeFillTempLines(SalesHeader);
        ...
        OnBeforePostLines(TempSalesLineGlobal, SalesHeader, SuppressCommit, PreviewMode, TempWhseShptHeader);
    ...
    begin
        with SalesHeader do begin
            ...
            if IsCreditDocType and GLSetup.GSTEnabled("Document Date") and GLSetup."Adjustment Mandatory" then
                TestField("Reason Code", ErrorInfo.Create());
    ...
    begin
        with SalesHeader do begin
            Ship := "Document Type" in ["Sales Document Type"::Order, "Sales Document Type"::Invoice];
            Receive := "Document Type" in ["Sales Document Type"::"Return Order", "Sales Document Type"::"Credit Memo"];
            Invoice := true;
        end;
    end;
    ...
    begin
        with SalesLine do begin
            ...
            OnPostSalesLineOnAfterRoundAmount(SalesHeader, SalesLine, SalesInvHeader, SalesCrMemoHeader);
    ...
    begin
        ...
        OnBeforePostGLAndCustomer2(SalesHeader, CustLedgEntry, SuppressCommit, PreviewMode, GenJnlPostLine, IsHandled);
#endif
        OnBeforePostInvoice(SalesHeader, CustLedgEntry, SuppressCommit, PreviewMode, GenJnlPostLine, IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        IsHandled := false;
        OnBeforePostItemLine(SalesHeader, SalesLine, IsHandled, TempSalesLineGlobal, TempDropShptPostBuffer, RemQtyToBeInvoiced, RemQtyToBeInvoicedBase);
        if IsHandled then
            exit;
    ...
    begin
        ...
        if FindTempItemChargeAssgntSales(SalesLineBackup."Line No.") then
            repeat
                ...
                OnPostItemChargeLineOnAfterTempItemChargeAssgntSalesLoop(TempItemChargeAssgntSales, SalesHeader, SalesLineBackup);
    ...
    begin
        IsHandled := false;
        OnBeforePostItemTrackingLine(SalesHeader, SalesLine, TempItemLedgEntryNotInvoiced, HasATOShippedNotInvoiced, IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            OnPostItemJnlLineOnBeforeIsJobContactLineCheck(ItemJnlLine, SalesHeader, SalesLine, ShouldPostItemJnlLine);
            ...
            OnAfterPostItemJnlLine(ItemJnlLine, SalesLine, SalesHeader, ItemJnlPostLine, WhseJnlPostLine, OriginalItemJnlLine, ItemShptEntryNo);
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            OnPostItemJnlLineOnAfterCopyTrackingFromSpec(ItemJnlLine, SalesLine, QtyToBeShipped);

            "Item Shpt. Entry No." := ItemLedgShptEntryNo;

            IsHandled := false;
            OnPostItemJnlLinePrepareJournalLineOnBeforeCalcQuantities(
                ItemJnlLine, SalesLine, QtyToBeShipped, QtyToBeShippedBase, QtyToBeInvoiced, QtyToBeInvoicedBase, IsHandled);
            if not IsHandled then begin
            ...
            OnPostItemJnlLinePrepareJournalLineOnBeforeCalcItemJnlAmounts(ItemJnlLine, SalesHeader, SalesLine);
            if QtyToBeInvoiced <> 0 then
        ...
        OnPostItemJnlLineOnAfterPrepareItemJnlLine(ItemJnlLine, SalesLine, SalesHeader, WhseShip, ItemJnlPostLine, QtyToBeShipped, TrackingSpecification);
    end;
    ...
    begin
        IsHandled := false;
        OnBeforeCalcItemJnlAmountsFromQtyToBeShipped(ItemJnlLine, SalesHeader, SalesLine, QtyToBeShipped, IsHandled, InvDiscAmountPerShippedQty);
        if IsHandled then
            exit;
    ...
    begin
        ...
        OnAfterPostItemJnlLineBeforePost(ItemJnlLine, SalesLine, QtyToBeShippedBase, ItemJnlPostLine);
    end;
        ...
        if not IsHandled then
            if DistributeCharge then
                ...
                PostItemCharge(
                    ...
                    TempItemChargeAssgntSales."Amount to Assign", TempItemChargeAssgntSales."Qty. to Assign");
    end;
    ...
    begin
        IsHandled := false;
        OnBeforeTestUpdatedSalesLine(SalesLine, IsHandled);
        if IsHandled then
            exit;
            ...
            if FindSet() then begin
                ...
                repeat
                    if CompleteFunctionality then begin
                        ...
                        OnCreatePrepaymentLinesOnBeforeValidateQtyToInvoice(SalesHeader, ShouldValidateQtyToInvoice);
                        ...
                        if TempLineFound then begin
                            ...
                            if TempExtTextLine.Find('-') then
                                repeat
                                    ...
                                    TempPrepmtSalesLine."Line No." := NextLineNo;
                                    NextLineNo := NextLineNo + 10000;
                                    OnCreatePrepaymentLinesOnBeforeInsertTempLineForExtText(TempPrepmtSalesLine, TempSalesLine);
                                    TempPrepmtSalesLine.Insert();
    ...
    begin
        IsHandled := false;
        OnBeforeArchiveUnpostedOrder(SalesHeader, IsHandled, PreviewMode);
        if IsHandled then
            exit;
    ...
    begin
        ...
        if not IsHandled then
            InvoicePostingInterface := "Sales Invoice Posting"::"Invoice Posting (v.19)";
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            Amount := -TotalSalesLine2."Amount Including VAT" + SalesHeader."WHT Amount";
            "Source Currency Amount" := -TotalSalesLine2."Amount Including VAT" + SalesHeader."WHT Amount";
            if (SalesHeader."WHT Amount" <> 0) and (SalesHeader."Currency Code" <> '') then
                ...
                "Amount (LCY)" := -(TotalSalesLineLCY2."Amount Including VAT" - SalesHeader."WHT Amount");
        ...
        if TempInvoicePostBufferGST.FindSet() then
            repeat
                with TempInvoicePostBufferGST do begin
                    ...
                    GSTSalesEntry."Document Line Type" := "Sales Line Type".FromInteger(Type);
    ...
    begin
        ...
        with SalesLine do begin
            ...
            if "Qty. to Invoice" <> 0 then begin
                InvoicePostBuffer.Type := Type.AsInteger();
                        ...
                        then begin
                            if "Amount Including VAT" < Amount then begin
                                InvoicePostBuffer."VAT Amount" :=
                                  Round("Amount Including VAT" - InvoicePostBuffer."VAT Base Amount", Currency."Amount Rounding Precision");
                                ...
                                InvoicePostBuffer."VAT Amount (ACY)" :=
                                  -Round(InvoicePostBuffer."VAT Base Amount" * "VAT %" / 100, Currency."Amount Rounding Precision");
                                InvoicePostBuffer."VAT Amount(ACY)" :=
                                  -Round(InvoicePostBuffer."VAT Base Amount" * "VAT %" / 100, Currency."Amount Rounding Precision");
                            end;
                            if "Amount Including VAT" > Amount then begin
                ...
                case SalesSetup."Discount Posting" of
                    SalesSetup."Discount Posting"::"Invoice Discounts":
                        begin
                            InvoicePostBuffer.Amount += "Inv. Discount Amount";
                    ...
                    SalesSetup."Discount Posting"::"Line Discounts":
                        begin
                            InvoicePostBuffer.Amount += "Line Discount Amount";
    ...
    [IntegrationEvent(TRUE, false)]
    local procedure OnAfterPostItemJnlLine(var ItemJournalLine: Record "Item Journal Line"; SalesLine: Record "Sales Line"; SalesHeader: Record "Sales Header"; var ItemJnlPostLine: Codeunit "Item Jnl.-Post Line"; var WhseJnlPostLine: Codeunit "Whse. Jnl.-Register Line"; OriginalItemJnlLine: Record "Item Journal Line"; var ItemShptEntryNo: Integer)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeArchiveUnpostedOrder(var SalesHeader: Record "Sales Header"; var IsHandled: Boolean; PreviewMode: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterPostItemJnlLineBeforePost(var ItemJournalLine: Record "Item Journal Line"; SalesLine: Record "Sales Line"; QtyToBeShippedBase: Decimal; var ItemJnlPostLine: Codeunit "Item Jnl.-Post Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCalcItemJnlAmountsFromQtyToBeShipped(var ItemJnlLine: Record "Item Journal Line"; SalesHeader: Record "Sales Header"; SalesLine: Record "Sales Line"; QtyToBeShipped: Decimal; var IsHandled: Boolean; var InvDiscAmountPerShippedQty: Decimal)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostGLAndCustomer2(var SalesHeader: Record "Sales Header"; var CustLedgerEntry: Record "Cust. Ledger Entry"; CommitIsSuppressed: Boolean; PreviewMode: Boolean; var GenJnlPostLine: Codeunit "Gen. Jnl.-Post Line"; var IsHandled: Boolean)
    begin
    end;
#endif
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeTestUpdatedSalesLine(SalesLine: Record "Sales Line"; var IsHandled: Boolean)
    begin
    end;
    ...
    begin
        ...
        OnAfterSalesShptLineInsert(SalesShptLine, SalesLine, ItemLedgShptEntryNo, WhseShip, WhseReceive, SuppressCommit, SalesInvHeader, TempWhseShptHeader, TempWhseRcptHeader, SalesShptHeader);
    end;
    ...
    [IntegrationEvent(true, false)]
    local procedure OnBeforePostItemTrackingLine(SalesHeader: Record "Sales Header"; SalesLine: Record "Sales Line"; var TempItemLedgEntryNotInvoiced: Record "Item Ledger Entry" temporary; HasATOShippedNotInvoiced: Boolean; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCreatePrepaymentLinesOnBeforeInsertTempLineForExtText(var TempPrepmtSalesLine: Record "Sales Line" temporary; var TempSalesLine: Record "Sales Line" temporary)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnCodeOnBeforeFillTempLines(var SalesHeader: Record "Sales Header")
    begin
    end;
...
#if not CLEAN19
    [Obsolete('Removed as part of Invoicing', '19.0')]
    [IntegrationEvent(false, false)]
    ...
    [IntegrationEvent(false, false)]
    local procedure OnSumSalesLines2SetFilter(var SalesLine: Record "Sales Line"; SalesHeader: Record "Sales Header"; InsertSalesLine: Boolean; QtyType: Option)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnFinalizePostingOnBeforeGenJnlPostPreviewThrowError(SalesHeader: Record "Sales Header"; SalesInvHeader: Record "Sales Invoice Header"; SalesCrMemoHeader: Record "Sales Cr.Memo Header")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostItemJnlLineOnBeforeIsJobContactLineCheck(var ItemJnlLine: Record "Item Journal Line"; SalesHeader: Record "Sales Header"; SalesLine: Record "Sales Line"; var ShouldPostItemJnlLine: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostItemJnlLinePrepareJournalLineOnBeforeCalcItemJnlAmounts(var ItemJnlLine: Record "Item Journal Line"; SalesHeader: Record "Sales Header"; var SalesLine: Record "Sales Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostSalesLineOnAfterRoundAmount(SalesHeader: Record "Sales Header"; SalesLine: Record "Sales Line"; SalesInvHeader: Record "Sales Invoice Header"; SalesCrMemoHeader: Record "Sales Cr.Memo Header")
    begin
    end;
    ...
    [IntegrationEvent(true, false)]
    local procedure OnPostItemChargeLineOnAfterTempItemChargeAssgntSalesLoop(var TempItemChargeAssgntSales: Record "Item Charge Assignment (Sales)" temporary; SalesHeader: Record "Sales Header"; var SalesLine: Record "Sales Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePostItemLine(SalesHeader: Record "Sales Header"; SalesLine: Record "Sales Line"; var IsHandled: Boolean; var TempSalesLineGlobal: Record "Sales Line" temporary; var TempDropShptPostBuffer: Record "Drop Shpt. Post. Buffer" temporary; var RemQtyToBeInvoiced: Decimal; var RemQtyToBeInvoicedBase: Decimal)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnRunOnBeforeCalcVATAmountLines(var TempSalesLineGlobal: Record "Sales Line" temporary; SalesHeader: Record "Sales Header"; var TempVATAmountLine: Record "VAT Amount Line" temporary; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeInsertShipmentLine(SalesHeader: Record "Sales Header"; SalesLine: Record "Sales Line"; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostItemJnlLinePrepareJournalLineOnBeforeCalcQuantities(var ItemJnlLine: Record "Item Journal Line"; SalesLine: Record "Sales Line"; QtyToBeShipped: Decimal; QtyToBeShippedBase: Decimal; QtyToBeInvoiced: Decimal; QtyToBeInvoicedBase: Decimal; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostItemJnlLineOnAfterCopyTrackingFromSpec(var ItemJnlLine: Record "Item Journal Line"; SalesLine: Record "Sales Line"; QtyToBeShipped: Decimal)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnCreatePrepaymentLinesOnBeforeValidateQtyToInvoice(var SalesHeader: Record "Sales Header"; var ShouldValidateQtyToInvoice: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPostInvoiceOnBeforePostBalancingEntry(var SalesHeader: Record "Sales Header"; var IsHandled: Boolean)
    begin
    end;
